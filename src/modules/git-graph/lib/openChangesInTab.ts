 ...
 *
 * A single commit reads as the range from its first parent, which is what "the
...
export function openChangesInTab(
  ...
  const [parent] = selection.commit.parents;
  if (parent) {
    return () => readComparison(repo, parent, selection.commit.hash);
  }
  return undefined;
}
